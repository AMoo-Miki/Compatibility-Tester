...
import { copyFile, readdir, readFile, rename, rm } from 'fs/promises';
import { _appendToFile, _changeInFile, _deleteFromFile, _download, _exec, _spawn, _unarchive } from './utils.js';
import { spawn } from 'child_process';
import { existsSync, rmSync } from 'fs';
import { ARCH, dashboardsPlugins, EXTENSION, LOG_PROPS, params, PLATFORM, tmpDir, TYPE } from './params.js';
...
export const downloadDashboards = async (version) => {
  if (version.startsWith('git://')) {
    ...
    versions.dashboards = `git://${user}/${repo}/${branch}@${clonedHash.trim()}`;
    ...
    for (const key in dashboardsPlugins) {
      ...
      const [, , user, repo, branch] = (params.get(`--${key}-version`) || dashboardsPlugins[key]).split('/');
      ...
      versions[key] = `git://${user}/${repo}/${branch}@${clonedHash.trim()}`;
    }
  ...
  const url = `https://ci.opensearch.org/ci/dbc/distribution-build-opensearch-dashboards/${version}/latest/${PLATFORM}/${ARCH}/${TYPE}/dist/opensearch-dashboards/opensearch-dashboards-${version}-${PLATFORM}-${ARCH}.${EXTENSION}`;
  return {
    dest: await _download(url, dest),
    versions: {
      dashboards: `${PLATFORM}-${ARCH}@${version}`,
    },
  };
};
...
export const prepareDashboards = async () => {
  ...
  if (osdDir) {
    ...
    const version = params.get('--dashboards-version') || 'git://opensearch-project/OpenSearch-Dashboards/main';
