...
import { copyFile, cp, mkdir, readdir, readFile, rename, rm } from 'fs/promises';
...
export const enrichNodeModules = async (testDir, osdDir) => {
  await cp(path.join(testDir, 'dashboards', 'node_modules'), path.join(osdDir, 'node_modules'), { force: false, recursive: true });
}
