...
import { LOG_PROPS, params, slugs } from '../params.js';
  ...
  for (const item of resultContent) {
    ...
    for (const run of runs) {
      ...
      results.forEach(result => {
        if (result.state in count) count[result.state]++;
      });
      ...
      if (scope === 'dashboards' && params.has('--no-plugins')) {
        scope = 'dashboards-no-plugins';
      }
