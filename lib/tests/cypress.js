...
export const runCypressTests = async (folder, root) => {
  ...
  for (const cypressRoot of cypressRoots) {
    ...
    processors.push(uploadArtifacts(cypressRoot, ref));
    /*
    if (params.has('--artifacts')) {
      const artifactDest = params.get('--artifacts');
      if (artifactDest !== true) { // A value is provided for --artifacts
        processors.push(moveArtifacts(cypressRoot, path.join(artifactDest, ref)));
      }
    }
     */
  }
...
/*
const moveArtifacts = async (folder, dest) => {
  ...
  _info(`Moving artifacts to ${dest}`);

  const screenshotsDir = path.join(cypressDir, 'screenshots');
  if (existsSync(screenshotsDir))
    await rename(screenshotsDir, path.join(dest, 'screenshots'));

  const videosDir = path.join(cypressDir, 'videos');
  if (existsSync(screenshotsDir))
    await rename(videosDir, path.join(dest, 'videos'));
};
*/

const uploadArtifacts = async (folder, ref) => {
  let cypressDir = path.join(folder, 'cypress');
  if (!existsSync(cypressDir))
    cypressDir = path.join(folder, '.cypress');
  if (!existsSync(cypressDir))
  ...
  const screenshotsDir = path.join(cypressDir, 'screenshots');
  if (existsSync(screenshotsDir)) {
    _notice(`Uploading screenshots to s3://${config.get('tests.artifacts.storage')}/${ref}`);
    await _spawn(`aws s3 cp ${screenshotsDir} s3://${config.get('tests.artifacts.storage')}/${ref} --recursive`);
  }

  const videosDir = path.join(cypressDir, 'videos');
  if (existsSync(videosDir)) {
    _notice(`Uploading videos to s3://${config.get('tests.artifacts.storage')}/${ref}`);
    await _spawn(`aws s3 cp ${videosDir} s3://${config.get('tests.artifacts.storage')}/${ref} --recursive`);
  }
};
