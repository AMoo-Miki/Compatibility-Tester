...
export const runCypressTests = async (folder, root) => {
  ...
  const artifactPrefix = params.has('--no-plugins')
    ? 'no-plugins'
    : params.has('--disable-security')
      ? 'no-security'
      : 'with-security';
  const processors = [];
  for (const cypressRoot of cypressRoots) {
    ...
    processors.push(uploadArtifacts(cypressRoot, ref, artifactPrefix + '/' + srcLabel));
  }
...
export const runParallelCypressTests = async (folder, builds, timestamp, ref) => {
  ...
  for (const cypressRoot of cypressRoots) {
    ...
    Object.keys(specGroups).forEach(key => {
      ...
      if (!/dashboards[\/\\]plugins/.test(cypressRoot) && !/^cypress[\/\\][^\/\\]+[\/\\]plugins/.test(key))
        noPluginSpecs.push({
          key,
          root: relPath,
          specs: specGroups[key],
  ...
  params.forEach((value, key) => {
    if (!['--parallel', '--no-clean', '--test-type', '--ref', '--timestamp', '--no-plugins', '--disable-security'].includes(key))
      jobParams.push(`${key}=${value}`);
  });

  const shouldRunWithSecurity = ['both', 'false'].includes(params.get('--disable-security'));
  const shouldRunWithoutSecurity = ['both', 'true'].includes(params.get('--disable-security'));

  const allSpecsCount = allSpecs.length;
  const noPluginSpecsCount = noPluginSpecs.length;
  const totalBuilds = builds.length +
    (shouldRunWithSecurity ? allSpecsCount : 0) +
    (shouldRunWithoutSecurity ? allSpecsCount : 0) +
    noPluginSpecsCount;

  const _runJob = async (job, noPlugins = false, withoutSecurity = false) => {
    const params = [...jobParams];
    if (noPlugins) params.push(`--no-plugins=true`);
    else params.push(`--disable-security=${withoutSecurity}`);

    const buildId = await triggerCypressBuild(job, params.join(' '), ref);
    if (buildId) {
      builds.push(buildId);
      _ok(`Started (${builds.length}: ${++cnt}/${totalBuilds}) ${buildId}`);
      _verbose(`        ${buildId}: Cypress tests in ${job.key} [${noPlugins ? 'no-plugins' : 'with-plugins'}, ${withoutSecurity ? 'no-security' : 'with-security'}]`)
    }
  ...
  for (const job of allSpecs) {
    if (shouldRunWithoutSecurity) await _runJob(job, false, true);
    if (shouldRunWithSecurity) await _runJob(job, false, false);
  }

  for (const job of noPluginSpecs) {
    await _runJob(job, true);
  }
};
