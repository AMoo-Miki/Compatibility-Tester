...
export const runParallelMochaTests = async (folder, builds, timestamp, ref) => {
  ...
  for (let job = 0; job <= maxGroup; job++) {
    const buildId = await triggerMochaBuild(job, [...jobParams, '--disable-security'].join(' '), ref);
    if (buildId) {
      builds.push(buildId);
    ...
    const buildNoPluginsId = await triggerMochaBuild(job, [...jobParams, '--no-plugins'].join(' '), ref);
    if (buildNoPluginsId) {
      builds.push(buildNoPluginsId);
      _ok(`Started (${builds.length}: ${++cnt}/${totalBuilds}) ${buildNoPluginsId}`);
    }
