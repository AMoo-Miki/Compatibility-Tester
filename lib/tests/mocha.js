...
import { _error, _info, _ok, _verbose } from '../logging.js';
...
export const runParallelMochaTests = async (folder, builds, timestamp, ref) => {
  ...
  const maxGroup = 13;
  const totalBuilds = maxGroup + maxGroup + builds.length;
  ...
  const _runJob = async (job, noPlugins = false, withoutSecurity = false) => {
    ...
    const buildId = await triggerMochaBuild(job, params.join(' '), ref);
    if (buildId) {
      builds.push(buildId);
      _ok(`Started (${builds.length}: ${++cnt}/${totalBuilds}) ${buildId}`);
      _verbose(`        ${buildId}: Mocha tests in group ${job} [${noPlugins ? 'no-plugins' : 'with-plugins'}, ${withoutSecurity ? 'no-security' : 'with-security'}]`)
    }
  ...
  for (let job = 0; job <= maxGroup; job++) {
    await _runJob(job, false, false);
    await _runJob(job, true, false);
  }
};